...
def forecast(data: ForecastRequest):
    ...
    try:
        ...
        growth_text = f"{growth_value:+.1f}%"

        frontend_forecast = []

        for item in daily_forecast:
            frontend_forecast.append(
                {
                    "date": item["date"],
                    "month": item["date"],
                    "value": round(
        ...
        return {
            ...
            "minimum_daily_sales": round(
                float(
                    summary["minimum_daily_sales"]
                ),
                2
            ),
            "maximum_daily_sales": round(
                float(
                    summary["maximum_daily_sales"]
                ),
                2
            ),
            ...
            "forecast_start": summary["start_date"],
            "forecast_end": summary["end_date"],
